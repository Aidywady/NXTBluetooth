/*
  arduino_sends_message

  Makes the arduino send a message to each of the 10 LEGO Mindstorms NXT inboxes, the NXT then displays the messages on its LCD screen.
  ...
  Documents\Arduino\libraries\NXTBluetooth\examples\arduino_sends_messages). The code must be opened, compiled, and uploaded using the
